defmodule GameOfLifeCore.Matrix.StateAgentTest do
  use ExUnit.Case
  doctest GameOfLifeCore

  alias GameOfLifeCore.Matrix.{StateAgent, StateBuilder}
  ...
  test "cell_and_environment" do
    ...
    {res, _pid} = StateAgent.cell_and_environment(0, 1)
    assert res == {{0, 0, 0},
                   {1, 0, 0},
                   {1, 1, 0}}
    ...
    {res, _pid} = StateAgent.cell_and_environment(2, 1)
    assert res == {{1, 1, 0},
                   {1, 0, 0},
                   {0, 0, 0}}

    {res, _pid} = StateAgent.cell_and_environment(2, 2)
    assert res == {{1, 0, 0},
                   {0, 0, 0},
                   {0, 0, 0}}
  end
  ...
  test "cell_and_environment 2" do
    ...
    {res, _pid} = StateAgent.cell_and_environment(0, 0)
    assert res == {{0, 0, 0},
                   {0, 0, 0},
                   {0, 1, 1}}
  end
  ...
  test "cell_and_environment 3" do
    ...
    {res, _pid} = StateAgent.cell_and_environment(2, 3)
    assert res == {{0, 1, 1},
                   {0, 1, 6},
                   {0, 0, 0}}
  end
  ...
  test "cell_and_env_line_list" do
    ...
    res = StateAgent.cell_and_env_line_list(line_count - 1, col_count - 1)
    assert length(res) == 4
  end
  ...
  test "cell_and_env_list" do
    ...
    res = StateAgent.cell_and_env_list(line_count - 1, col_count - 1)
