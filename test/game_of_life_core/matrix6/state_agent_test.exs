defmodule GameOfLifeCore.Matrix6.StateAgentTest do
  use ExUnit.Case
  doctest GameOfLifeCore

  alias GameOfLifeCore.Matrix6.{StateAgent, StateBuilder}
  ...
  describe "update_servers_with_neighbors" do
    test 'simple board' do
      [[1, 0, 0, 1],
      [1, 1, 0, 1],
      [1, 0, 0, 1]]
     |> StateBuilder.build_state
     |> StateBuilder.update_servers_with_neighbors
     |> StateAgent.start_link

     board = StateAgent.state()
     {pid, env_pids} = elem(elem(board, 0), 0)
     assert is_pid(pid)
     assert is_list(env_pids)
     assert Enum.all?(env_pids, fn x -> is_pid(x) end)
    end
  end
